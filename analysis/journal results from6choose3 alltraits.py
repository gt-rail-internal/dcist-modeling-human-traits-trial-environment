...
if PLOTTING:
    ...
    # load the list
    with open("f6c3onlys1s2_results.pkl", "rb") as f:
    ...
    plot = ax.hist(bins[:-1], bins, weights=counts / 120, range=range, alpha=alpha, color="orange", label="All Possibilities")
    #plot = ax.hist(all_assignments, bins=bins, range=range, alpha=alpha - .1, color="grey", label="All Possibilities", weights=[1/6 for x in range(bins)])
    ...
    ax.legend()
    ax.set_title("[From 6 Choose 3 ONLY STAGE 1 and 2] Histogram of Scores via Assignment Methods with all Traits Applied, N=" + str(len(data[0])))
...
for n in range(N):
    ...
    for team in team_indexes:
        ...
        score_data.append([best_value, pred_value, adjusted_pred_value, actual_value, worst_value, expected_value, [x for x in all_assignments]])  # record the score data
    ...
    for i in range(len(score_data)):
        ...
        all_assignments += score_data[i][-1]
    expected_value /= len(score_data)
    

    iteration_scores.append([best_score, actual_score, worst_score, expected_value, all_assignments])
    #print("TEAM OUTCOME")
...
with open("f6c3onlys1s2_results.pkl", "wb") as f:
